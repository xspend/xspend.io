...
from models import BudgetHistory, ChatLog
from models import (
...
def chat_options(db: Session = Depends(get_db), current_user: str = Depends(get_current_user)):
    """Everything the chat UI needs in one call."""
    from ai_chat import months_with_data
...
def chat_prompt(data: ChatPromptRequest, db: Session = Depends(get_db),
                current_user: str = Depends(get_current_user)):
    from ai_chat import prompt_dispatch
